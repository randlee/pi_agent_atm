...
## Relationship to `pi-agent-rust`

This repo also has a `pi-agent-rust` skill (`.claude/skills/pi-agent-rust/SKILL.md`) that owns
repo-specific bootstrap commands, the symptom router, `tests/installer_regression.sh`,
`scripts/skill-smoke.sh`, and done-criteria for this codebase. Use `pi-agent-rust` for
"how do I quickly verify a change in this repo" questions. Use `rust-development` for
architecture planning, code review, and code exploration delegated to the
`rust-architect` / `rust-code-reviewer` / `rust-code-explorer` / `rust-developer` agents, and for
general Pragmatic Rust Guidelines compliance that applies beyond this one repo. When the two
skills both touch build/lint commands (e.g. clippy invocation), defer to `pi-agent-rust`'s
`rch exec --` wrapper for this repo's actual gate commands; this skill's guidelines are the
style/architecture reference, not a competing bootstrap procedure.
