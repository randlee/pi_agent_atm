...
```bash
grep -rn '"/tmp/' src/ && echo "FAIL: Found /tmp hardcoding" || echo "OK"
```
...
For heavy compile/lint workloads in this repo, prefer the `rch exec --` wrapper described in the
`pi-agent-rust` skill (e.g. `rch exec -- cargo clippy --all-targets -- -D warnings`) rather than
