...
```json
{
  ...
  "branch": "feature/mailbox-locking",
  "commit": "abc1234",
  "pr": 123,
  "verdict": "FAIL",
  "deliverables": {
...
This repo uses git-flow with `develop` as the integration branch and `main` as
the protected release branch. Feature/fix branches open PRs against `develop`;
`develop` is promoted to `main` for releases. QA passes described below apply
to any PR regardless of its target branch — treat `branch` in the status
contract as the source branch and note the PR's target (`develop` or `main`)
in `next_action`/`merge_reason` when it affects readiness (e.g. a PR into
`main` may require the additional release checks the team has defined
elsewhere; this skill does not define those checks).
