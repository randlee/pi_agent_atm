...
```markdown
## Crate Boundary Review: `pi_agent_rust`

### `pub trait Provider`
- Implementations: Anthropic, OpenAI, Gemini, Bedrock, Vertex, Azure, Cohere, Copilot, GitLab (fixed, first-party)
- External implementation expected: No
- **Recommendation: SEAL**
- Rationale: Fixed set of backend integrations owned by this crate. Sealing allows adding
  methods (e.g., `fn supports_prompt_caching(&self) -> bool`) without a breaking change.

### `pub trait Tool`
- Implementations: built-ins (`read`, `write`, `edit`, `bash`, `grep`, `find`, `ls`) plus
  extension-registered tools (QuickJS host calls)
- External implementation expected: Yes (extensions register custom tools)
...
/// # Sealed
///
/// This trait is sealed and cannot be implemented outside of `pi_agent_rust`.
/// This allows the trait to evolve (new methods, changed signatures)
/// without breaking downstream consumers.
///
/// If you need custom request/response handling, configure an existing
/// provider or open an issue to add a new first-party backend.
