...
```bash
atm send cpi --stdin < /tmp/step-1-message.xml
```
...
- `FAIL` -> re-render and re-send Step 1 to `cpi`
- if `cpi` ACKs but returns no new fenced JSON, increment `round_index`,
