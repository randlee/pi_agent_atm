# Step 3 — Sprint Scope Hardening (`cpi`)
...
```bash
atm send cpi --stdin < /tmp/step-3-message.xml
```

**3. Check the response**

Read the `cpi` response and confirm it contains fenced JSON.
...
- `FAIL` -> re-render and re-send Step 3 to `cpi`
- if `cpi` ACKs but responds as though the same already-fixed round is
